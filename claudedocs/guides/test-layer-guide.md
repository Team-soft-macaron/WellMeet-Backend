...
> WellMeet-Backend 프로젝트의 테스트 레이어별 구성 및 작성 표준

## 📚 목차

1. [Entity Layer (domain-* 모듈)](#1-entity-layer-domain--모듈)
2. [Repository Layer (domain-* 모듈)](#2-repository-layer-domain--모듈)
3. [Domain Service Layer (domain-reservation 모듈)](#3-domain-service-layer-domain-reservation-모듈)
4. [Service Layer (api-user, api-owner 모듈)](#4-service-layer-api-user-api-owner-모듈)
5. [Controller Layer (api-user, api-owner 모듈)](#5-controller-layer-api-user-api-owner-모듈)
6. [Redis Service Layer (infra-redis 모듈)](#6-redis-service-layer-infra-redis-모듈)
7. [Kafka Producer Layer (infra-kafka 모듈)](#7-kafka-producer-layer-infra-kafka-모듈)
8. [Batch Job Layer (batch-reminder 모듈)](#8-batch-job-layer-batch-reminder-모듈)
